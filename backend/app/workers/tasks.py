...
def run_daily_screener():
    """Daily Celery job: screen stocks and trigger AI pipeline for candidates."""
    result = run_quantitative_screen()
    triggered = 0
    for candidate in result.get("candidates", []):
        trigger_analysis_pipeline.delay(candidate["symbol"], result.get("run_id"))
        triggered += 1
...
def trigger_analysis_pipeline(ticker_symbol: str, screening_run_id: str | None = None):
    """Run the LangGraph multi-agent pipeline for a single ticker."""
    return run_pipeline(ticker_symbol, screening_run_id)
